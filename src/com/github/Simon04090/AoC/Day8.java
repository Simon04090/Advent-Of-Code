...
public class Day8 {
    private static final String input = "o inc 394 if tcy >= -3\n" +
            "s inc -204 if s <= -5\n" +
            "keq inc -34 if keq > -6\n" +
            "ip inc 762 if ip == 4\n" +
            "zr inc -644 if ip >= 6\n" +
    ...
    private static final HashMap<Integer, String> actionMap = new HashMap<>();
    private static final HashMap<Integer, Integer> amountMap = new HashMap<>();
    private static final HashMap<Integer, String> conditionReferenceMap = new HashMap<>();
    private static final HashMap<Integer, String> conditionOperationMap = new HashMap<>();
    private static final HashMap<Integer, Integer> conditionAmountMap = new HashMap<>();
    private static final HashMap<String, Integer> valueMap = new HashMap<>();
    private static final HashMap<Integer, String> registerMap = new HashMap<>();
    ...
    private static Integer part1() {
        return valueMap.values().stream().max(Integer::compareTo).orElse(0);
    }
    ...
    private static void processInput() {
        for (int i = 0; i < actionMap.size(); i++) {
            ...
            String action = actionMap.get(i);
            Integer amount = amountMap.get(i);


            if (parseCondition(conditionReferenceMap.get(i), conditionOperationMap.get(i), conditionAmountMap.get(i))) {
                int value = 0;
                if (action.equals("inc")) {
                    String key = registerMap.get(i);
                    value = valueMap.get(key) + amount;
    ...
    private static boolean parseCondition(String conditionReference, String conditionOperation, Integer conditionAmount) {
        boolean condition;
        switch (conditionOperation) {
            case "==":
                condition = valueMap.get(conditionReference).equals(conditionAmount);
                break;
            case "!=":
                condition = !valueMap.get(conditionReference).equals(conditionAmount);
                break;
            case ">":
                condition = valueMap.get(conditionReference).compareTo(conditionAmount) >= 1;
                break;
            case "<":
                condition = valueMap.get(conditionReference).compareTo(conditionAmount) <= -1;
                break;
            case "<=":
                condition = valueMap.get(conditionReference).compareTo(conditionAmount) <= 0;
                break;
            case ">=":
                condition = valueMap.get(conditionReference).compareTo(conditionAmount) >= 0;
                break;
    ...
    private static void prepareInput() {
        ...
        for (int i = 0; i < instructions.length; i++) {
            ...
            registers.add(instruction[0]);
            registers = (ArrayList<String>) registers.stream().distinct().collect(Collectors.toList());
            actionMap.put(i, instruction[1]);
            amountMap.put(i, Integer.valueOf(instruction[2]));
            conditionReferenceMap.put(i, instruction[4]);
            conditionOperationMap.put(i, instruction[5]);
            conditionAmountMap.put(i, Integer.valueOf(instruction[6]));
            registerMap.put(i, instruction[0]);
        }
        for (String instruction : registers) {
            valueMap.put(instruction, 0);
        }
    }
